...
import com.elephasvacation.tms.web.api.roomType.RoomTypeAPI;
...
public class RoomTypeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        int roomTypeID;
        RoomTypeAPI roomTypeAPI = APIFactory.getInstance().getAPI(APITypes.ROOM_TYPE);
        ...
        if (request.getPathInfo() == null || request.getPathInfo()
            ...
            try (Connection connection = bds.getConnection();) {
                roomTypeAPI.setConnection(connection);
                List<RoomTypeDTO> allRoomTypes = roomTypeAPI.getAllRoomTypes();
                out.println(jsonb.toJson(allRoomTypes));
        ...
        /* get roomType by roomTypeID. */
            ...
            /* extracting RoomTypeID from URL. */
            roomTypeID = IDUtil.extractIDFrom(splitURIArray,
                    Number.ONE,
                    "rt",
                    "Invalid RoomTypeID");
            ...
            try (Connection connection = bds.getConnection();) {
                ...
                roomTypeAPI.setConnection(connection);
                RoomTypeDTO roomTypeDTO = roomTypeAPI.getRoomTypeByID(roomTypeID);
                if (roomTypeDTO == null) {
                    response.sendError(HttpServletResponse.SC_NOT_FOUND);
                } else {
                    out.println(jsonb.toJson(roomTypeDTO));
                }
            } catch (SQLException sqlException) {
                logger.error(FailedMessages.SOMETHING_WENT_WRONG_DATABASE_OPERATION, sqlException);
                sqlException.printStackTrace();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (request.getPathInfo() == null || request.getPathInfo()
            ...
            try {
                roomTypeDTO = jsonb.fromJson(request.getReader(), RoomTypeDTO.class);
            } catch (JsonbException jsonbException) {
                /* cannot parse the request header to an RoomType object. */
            ...
            try (Connection connection = bds.getConnection();) {
                roomTypeAPI.setConnection(connection);
                Integer generatedRoomTypeID = roomTypeAPI.createRoomType(roomTypeDTO);
                if (generatedRoomTypeID != null && generatedRoomTypeID > 0) {
                    /* RoomType created successfully. */
                    /* response-content-type. */
                    response.setContentType(MimeTypes.Application.JSON);
                    response.setStatus(HttpServletResponse.SC_CREATED);
                    out.println(jsonb.toJson(new CreatedOutputDTO(String.format("RT%03d", generatedRoomTypeID))));
                } else {
                    response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                }
    ...
    protected void doPut(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (request.getPathInfo() != null &&
            ...
            try (Connection connection = bds.getConnection();) {
                ...
                if (roomTypeAPI.getRoomTypeByID(roomTypeID) == null) {
                ...
                /* read the request body. */
                RoomTypeDTO roomTypeDTO = jsonb.fromJson(request.getReader(), RoomTypeDTO.class);
                roomTypeDTO.setId(roomTypeID);

                if (roomTypeAPI.updateRoomType(roomTypeDTO)) {
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (request.getPathInfo() != null &&
            ...
            try (Connection connection = bds.getConnection();) {
                ...
                /* delete record by ID. */
                if (roomTypeAPI.deleteRoomTypeByID(roomTypeID)) {
                    response.setStatus(HttpServletResponse.SC_NO_CONTENT);
                } else {
                    logger.error(FailedMessages.SOMETHING_WENT_WRONG);
                    response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
